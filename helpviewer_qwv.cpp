...
{
public:
    HelpPage(Documentacion *central, QObject *parent);
...
HelpPage::HelpPage(Documentacion *central, QObject *parent)
    : QWebPage(parent)
...
{
    TRACE_OBJ
    HelpPage* newPage = static_cast<HelpPage*>(documentacion->newEmptyTab()->page());
    if (newPage)
    ...
    const QNetworkRequest &request, QWebPage::NavigationType type)
{
    ...
    if (AbstractHelpViewer::launchWithExternalApp(url)) {
        if (closeNewTab)
            QMetaObject::invokeMethod(documentacion, "closeTab");
        return false;
    }
    ...
    if (type == QWebPage::NavigationTypeLinkClicked
        ...
        || m_pressedButtons == Qt::MidButton)) {
            if (documentacion->newEmptyTab())
                documentacion->setSource(url);
            m_pressedButtons = Qt::NoButton;
            m_keyboardModifiers = Qt::NoModifier;
            return false;
    }
...
HelpViewer::HelpViewer(Documentacion *parent, qreal zoom)
    : QWebView(parent)
    , parentWidget(parent)
    , loadFinished(false)
    , helpEngine(HelpEngineWrapper::instance())
{
    ...
    action->setText(tr("Abrir Link en Nuevo Tab"));
    if (!parent)
