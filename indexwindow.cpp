...
bool IndexWindow::eventFilter(QObject *obj, QEvent *e)
{
    ...
    if (obj == m_searchLineEdit && e->type() == QEvent::KeyPress) {
        ...
        if (idx.isValid()) {
            QMenu menu;
            QAction *curTab = menu.addAction(tr("Abrir Link"));
            QAction *newTab = menu.addAction(tr("Abrir Link en Nuevo Tab"));
...
void IndexWindow::open(QHelpIndexWidget* indexWidget, const QModelIndex &index)
{
    ...
    if (model) {
        ...
        if (!AbstractHelpViewer::canOpenPage(url.path()))
            Documentacion::instance()->setSource(url);
        else
            Documentacion::instance()->setSourceInNewTab(url);
    }
}
